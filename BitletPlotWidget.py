...
class PlotWindow(FigureCanvas):
    ...
    def draw_2d_graph(self):  # Function for graph plotting
        ...
        plt.colorbar(cf,ax=self.axes)
        self.axes.contour(x_axis, y_axis, z2,colors='red')
        self.axes.contour(x_axis, y_axis, z,colors='black')
        maxx = self.x_axis.max
        maxy = self.y_axis.max
        self.axes.set_xlim(xmin=self.x_axis.min , xmax=maxx)
        self.axes.set_ylim(ymin=self.y_axis.min, ymax=maxy)
...
class BitletPlotWidget(QWidget):
    ...
    def set_ui(self):
        ...
        self.curr_plot = '2d'
        self.canvas = PlotWindow(self.x_param,self.y_param,params,self.resolution)
    ...
    def update_resolution(self,n):
        ...
        self.canvas.resolution = selected
        self.canvas.draw_2d_graph()